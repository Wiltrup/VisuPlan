...
module.exports = async function handler(request, response) {
  ...
  try {
    ...
    if (action === 'request-subscription') {
      ...
      await service(`/rest/v1/customers?id=eq.${encodeURIComponent(customer.id)}`, secret, { method: 'PATCH', headers: { Prefer: 'return=minimal' }, body: JSON.stringify({ subscription_interest_at: now, updated_at: now }) });
      await notifyOwner(`Kunde vil fortsætte med VisuPlanner: ${customer.display_name}`, `Kunde: ${customer.display_name}\nTavle: ${team.name}\nKontakt: ${customer.contact_name || ''}\nMail: ${customer.contact_email || team.recovery_email}\n\nKontakt kunden om pakke, faktura og aktivering.`);
      return response.status(200).json({ ok: true, requested_at: now });
    }
