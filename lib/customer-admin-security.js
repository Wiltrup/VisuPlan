...
const SUPPORT_EMAIL = 'wiltrup@wiltrup.com';

// Sikkerhedsnet for ældre administrative mailflows, som sender direkte via Resend.
// Afsenderadressen må gerne være en teknisk VisuPlanner-adresse, men svar fra
// kunder skal altid lande i den rigtige indbakke.
if (typeof globalThis.fetch === 'function' && !globalThis.__VISUPLANNER_RESEND_REPLY_TO__) {
  ...
  globalThis.fetch = async (input, init = {}) => {
    ...
    if (url === 'https://api.resend.com/emails' && typeof init?.body === 'string') {
      try {
        ...
        if (!payload.reply_to) {
          init = { ...init, body: JSON.stringify({ ...payload, reply_to: SUPPORT_EMAIL }) };
        }
      } catch {}
    }
    return originalFetch(input, init);
  };
  globalThis.__VISUPLANNER_RESEND_REPLY_TO__ = true;
}
...
async function sendMail(to, subject, text) {
  ...
  try {
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body:JSON.stringify({ from:'VisuPlanner <velkommen@visuplanner.dk>', to:[to], reply_to:SUPPORT_EMAIL, subject, text })
    });
