...
function credentialHtml(item, kind, scope = 'team') {
  ...
  const awaitingActivation = item.onboarding_status !== 'active';
  ...
  const missingValue = awaitingActivation ? 'Vælges ved aktivering' : 'Skal gemmes igen én gang';
  const missingHelp = awaitingActivation
    ? 'Den ansvarlige medarbejder vælger koden via aktiveringslinket.'
    : 'Indtast koden igen nedenfor. Den må gerne være den samme som den nuværende.';
  return `<section class="credential-row" data-credential="${kind}"><div class="credential-title"><strong>${label}</strong><span class="credential-value ${hasCode ? '' : 'missing'}">${hasCode ? '••••••••' : missingValue}</span></div><small>${hasCode ? `Senest ændret ${dateTime(changed)}` : missingHelp}</small><div class="credential-actions">${hasCode ? `<button class="reveal-code" data-action="reveal-code" data-scope="${scope}" data-kind="${kind}" type="button">Vis kode</button>` : ''}</div><form class="credential-reset" ${resetAttribute}="${kind}"><input name="value" type="password" minlength="${minimum}" placeholder="Ny ${label.toLowerCase()} – mindst ${minimum} tegn" required><button type="submit">Vælg ny</button></form></section>`;
}
...
function clubHtml(offer) {
  ...
  const needsCredentialRefresh = active && (!offer.has_viewer_code || !offer.has_editor_code);
  const linkedTeams = new Set((offer.team_links || []).map(link => link.team_slug));
  const teamChoices = dashboard.teams.length
    ? dashboard.teams.map(team => `<label><input type="checkbox" name="team_slugs" value="${esc(team.slug)}" ${linkedTeams.has(team.slug) ? 'checked' : ''}> ${esc(team.name)}</label>`).join('')
    : '<p class="empty">Opret mindst én teamtavle for at gøre klubben tilgængelig.</p>';
  return `<article class="customer-board offer-card" data-offer="${esc(offer.id)}"><div class="board-head"><div><h3>${esc(offer.name)}</h3><code>visuplanner.dk${esc(offer.public_path)}</code></div><span class="badge ${active ? '' : 'pending'}">${active ? 'Aktiv' : 'Afventer aktivering'}</span></div><div class="board-meta"><span>${esc(offer.workplace || '')} · ${esc(dashboard.customer.municipality || '')}</span><span>Ansvarlig: ${esc(offer.recovery_email || '')}</span></div>${needsCredentialRefresh ? '<div class="credential-refresh-note"><strong>Gem klubbens koder én gang</strong><span>Denne klub blev aktiveret, før sikker kodevisning blev tilføjet. Indtast begge koder igen nedenfor – de må gerne være de samme som nu. Derefter kan de vises og kopieres her.</span></div>' : ''}<div class="credential-list">${credentialHtml(offer,'viewer','club')}${credentialHtml(offer,'editor','club')}</div><div class="board-actions">${active ? `<a href="${esc(offer.public_path)}" target="_blank" rel="noopener">Åbn klubtavle</a><button data-club-action="send-club-reset" type="button">Send nulstillingslink</button>` : '<button data-club-action="send-club-invite" type="button">Send aktiveringslink igen</button>'}<button data-club-action="delete-club" class="danger" type="button">Slet klubtavle</button></div><details class="team-edit"><summary>Ret kluboplysninger og tilknytning</summary><form data-save-club class="field-grid"><label>Klubnavn<input name="name" value="${esc(offer.name)}" required></label><label>Ansvarlig mail<input name="recovery_email" type="email" value="${esc(offer.recovery_email || '')}" required></label><label>Arbejdsplads<input name="workplace" value="${esc(offer.workplace || '')}" required></label><fieldset class="wide"><legend>Gør tilgængelig på disse tavler</legend><div class="club-team-choices">${teamChoices}</div><small>De valgte teams kan selv vælge at vise eller skjule klubben i Grundindstillinger.</small></fieldset><div class="form-actions"><button type="submit">Gem oplysninger og tilknytning</button></div></form></details></article>`;
}
...
function bindDashboardActions() {
  ...
  document.querySelectorAll('[data-save-club]').forEach(form => form.onsubmit = async event => {
    event.preventDefault(); const card = form.closest('[data-offer]'); const data = new FormData(form); const values = Object.fromEntries(data); values.team_slugs = data.getAll('team_slugs'); const button = event.submitter; button.disabled = true;
    try { await post({ action:'save-club', offer_id:card.dataset.offer, ...values }); notify('Kluboplysningerne og de tilgængelige tavler er gemt.'); await loadDashboard(); }
    catch (error) { notify(error.message,'error'); button.disabled = false; }
  });
