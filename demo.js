...
const DEMO_CLUB=[
  {meal:'Pastasalat og flute',activities:[['15:00','16:00','Kreativt værksted'],['17:00','18:00','Fælles kaffe og spil']]},
  {meal:'Wraps med kylling',activities:[['14:30','15:30','Musik og ønskesange'],['17:00','18:30','Brætspilscafé']]},
  {meal:'Tarteletter',photo:'/assets/demo/tarteletter.webp',activities:[['15:00','16:00','Kreativt værksted'],['17:00','18:30','Spil og fælles hygge']]},
  {meal:'Tarteletter',activities:[['15:30','16:30','Gåtur fra klubben'],['18:00','19:00','Quiz']]},
  {meal:'Pizza',activities:[['14:00','15:30','Fredagsværksted'],['19:00','21:00','Filmaften']]},
  {meal:'Sandwich',activities:[['13:00','15:00','Udflugt'],['16:00','17:00','Eftermiddagskaffe']]},
  {meal:'Suppe og brød',activities:[['14:00','15:00','Søndagsbanko'],['16:00','17:00','Rolig café']]}
];
const KEY='visuplanner-demo-session-v6';
...
],settings:{morning:2,evening:2,night:1,showDates:true,shiftMode:3,nightEnabled:true,showBreakfast:false,showLunch:false,showClub:true}});
...
function isoWeekNumber(value){const date=new Date(Date.UTC(value.getFullYear(),value.getMonth(),value.getDate())),weekday=date.getUTCDay()||7;date.setUTCDate(date.getUTCDate()+4-weekday);const yearStart=new Date(Date.UTC(date.getUTCFullYear(),0,1));return Math.ceil((((date-yearStart)/86400000)+1)/7)}
function demoWeekLabel(){return `Uge ${isoWeekNumber(monday())} · ${formatDate(dateAt(0))} – ${formatDate(dateAt(6))}`}
function demoPhotoNote(element,hasPhoto,selectedPhoto=false,detail=''){element.classList.toggle('is-empty',!hasPhoto);element.innerHTML=`<strong>${hasPhoto?(selectedPhoto?'Billede valgt ✓':'Billede tilknyttet ✓'):'Intet billede valgt'}</strong>${detail?`<span>${esc(detail)}</span>`:''}`}
...
function render(){
  ...
  const club=DEMO_CLUB[selected];
  $('demoSharedOffersSection').hidden=!demoState.settings.showClub;
  $('demoSharedOffersList').innerHTML=demoState.settings.showClub?`<article class="shared-offer-panel"><header class="shared-offer-name"><h3>Klub Solsikken</h3></header><div class="shared-offer-content"><section><h4>🍽️ Mad</h4>${club.photo?`<button class="shared-offer-photo image-button" data-demo-image="${club.photo}" data-demo-caption="${esc(club.meal)}"><img src="${club.photo}" alt=""></button>`:''}<strong>${esc(club.meal)}</strong></section><section><h4>🎯 Aktiviteter</h4>${club.activities.map(item=>`<div class="shared-offer-activity"><time>${item[0]}–${item[1]}</time><span>${esc(item[2])}</span></div>`).join('')}</section></div><p class="shared-offer-message">💬 Klubbens indhold opdateres direkte hos klubben og vises på alle tavler, der er tilknyttet klubben.</p></article>`:'';
...
function loadEditor(){
  ...
  $('demoWeekLabel').textContent=demoWeekLabel();
  ...
  ['breakfast','lunch','dinner'].forEach(type=>{const cap=type[0].toUpperCase()+type.slice(1);demoPhotoNote($(`demo${cap}PhotoNote`),Boolean(editMealPhotos[type]),false,editMealPhotos[type]?'Vælg et nyt billede for at udskifte det.':'Upload et billede, hvis I ønsker det.')});
...
function renderActivityEditor(){
  $('demoActivityEditor').innerHTML=editActivities.length?editActivities.map((activity,index)=>`<div class="demo-activity-edit"><input type="time" value="${esc(activity.time)}" data-demo-index="${index}" data-demo-field="time"><input value="${esc(activity.name)}" placeholder="Aktivitet" data-demo-index="${index}" data-demo-field="name"><label class="demo-photo-button">${activity.photo?'Vælg et andet billede':'Billede'}<input type="file" accept="image/jpeg,image/png,image/webp" data-demo-activity-photo="${index}"></label><button type="button" data-demo-remove="${index}">✕</button><div class="photo-selection-note ${activity.photo?'':'is-empty'}" style="grid-column:1/-1;margin:0"><strong>${activity.photo?'Billede tilknyttet ✓':'Intet billede valgt'}</strong><span>${activity.photo?'Vælg et nyt billede for at udskifte det.':'Upload et billede til aktiviteten, hvis I ønsker det.'}</span></div></div>`).join(''):'<p class="empty">Ingen aktiviteter endnu.</p>';
