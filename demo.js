...
const KEY='visuplanner-demo-session-v2';
const $=id=>document.getElementById(id);
const esc=value=>String(value||'').replace(/[&<>"']/g,character=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;',"'":'&#39;'}[character]));
const blankDay=()=>({morning:['',''],evening:['',''],night:['',''],dinner:'',dinnerPhoto:'',activities:[]});
const defaultState=()=>({week:DEMO_DAYS.map(blankDay),settings:{morning:2,evening:2,night:2,showDates:true}});
let demoState=loadState(),selected=(new Date().getDay()+6)%7,editActivities=[],editShifts={morning:[],evening:[],night:[]},editDinnerPhoto='';
...
function render(){
  ...
  renderPeople('demoMorning',day.morning);renderPeople('demoEvening',day.evening);renderPeople('demoNight',day.night);
  $('demoDinner').textContent=day.dinner||'Ikke udfyldt';
  $('demoDinnerPhoto').innerHTML=day.dinnerPhoto?`<img src="${day.dinnerPhoto}" alt="${esc(day.dinner||'Aftensmad')}" data-demo-image="${day.dinnerPhoto}" data-demo-caption="${esc(day.dinner||'Aftensmad')}">`:'';
...
function loadEditor(){
  ...
  $('demoDinnerInput').value=day.dinner;editDinnerPhoto=day.dinnerPhoto||'';
  $('demoDinnerPhotoNote').textContent=editDinnerPhoto?'Der er valgt et billede. Vælg et nyt for at udskifte det.':'Billedet behandles kun på denne enhed.';
...
$('demoDinnerPhotoInput').onchange=async event=>{if(!event.target.files?.[0])return;status('Behandler billedet lokalt…');editDinnerPhoto=await compressImage(event.target.files[0]);$('demoDinnerPhotoNote').textContent='Billedet er klar og gemmes kun i denne demosession.';status('Billedet er klar')};
$('demoSave').onclick=()=>{const day=demoState.week[selected];day.morning=[...editShifts.morning];day.evening=[...editShifts.evening];day.night=[...editShifts.night];day.dinner=$('demoDinnerInput').value.trim();day.dinnerPhoto=editDinnerPhoto;day.activities=editActivities.filter(activity=>activity.name.trim()).map(activity=>({...activity,name:activity.name.trim()}));if(!saveState())return;render();$('demoEditorDialog').close();status('Dagen er gemt i denne browserfane')};
...
$('demoOpenSettings').onclick=()=>{$('demoMorningDefault').value=demoState.settings.morning;$('demoEveningDefault').value=demoState.settings.evening;$('demoNightDefault').value=demoState.settings.night;$('demoShowDates').checked=demoState.settings.showDates;$('demoSettingsDialog').showModal()};
$('demoSettingsClose').onclick=()=>$('demoSettingsDialog').close();
$('demoSettingsSave').onclick=()=>{demoState.settings={morning:Math.min(10,Math.max(1,Number($('demoMorningDefault').value)||1)),evening:Math.min(10,Math.max(1,Number($('demoEveningDefault').value)||1)),night:Math.min(10,Math.max(1,Number($('demoNightDefault').value)||1)),showDates:$('demoShowDates').checked};demoState.week.forEach(ensureShiftDefaults);saveState();render();loadEditor();$('demoSettingsDialog').close();status('Grundindstillingerne er gemt')};
